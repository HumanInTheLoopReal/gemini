...
with a number (1-[N]) or `q` to exit." </step>
...
If `files` field has entries, read and briefly summarize each. </step>
...
**If `COMMIT_PLANNING_DOCS=false`:** Skip git operations, log "Todo moved (not
...
- Don't create plans from this command — route to /gsd:plan-phase or
  /gsd:add-phase </anti_patterns>
