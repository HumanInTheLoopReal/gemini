...
<core_principle> The orchestrator's job is coordination, not execution. Each
subagent loads the full execute-plan context itself. Orchestrator discovers
plans, analyzes dependencies, groups into waves, spawns agents, handles
checkpoints, collects results. </core_principle>

<required_reading> Read STATE.md before any operation to load project context.
Read config.json for planning behavior settings. </required_reading>
...
Report: "Found {N} plans in {phase_dir}" </step>
...
**No dependency analysis needed.** Wave numbers are pre-computed during
`/gsd:plan-phase`.
...
The "What it builds" column comes from skimming plan names/objectives. Keep it
brief (3-8 words). </step>
...
1. **Describe what's being built (BEFORE spawning):**
   ...
   Read each plan's `<objective>` section. Extract what's being built and why it
   matters.
...
2. **Read files and spawn all autonomous agents in wave simultaneously:**
   ...
   Before spawning, read file contents. The `@` syntax does not work across
   Task() boundaries - content must be inlined.
   ...
   Use Task tool with multiple parallel calls. Each agent gets prompt with
   inlined content:
...
3. **Wait for all agents in wave to complete:**
   ...
   Task tool blocks until each agent finishes. All parallel agents return
   together.
...
2. **Agent runs until checkpoint:**
   ...
   - Reaches checkpoint task (e.g., `type="checkpoint:human-verify"`) or auth
     gate
   - Agent returns with structured checkpoint (see checkpoint-return.md
     template)
...
4. **Orchestrator presents checkpoint to user:**
   ...
   Extract and display the "Checkpoint Details" and "Awaiting" sections from
   agent return:
...
6. **Spawn continuation agent (NOT resume):**
   ...
   - `{resume_instructions}`: Based on checkpoint type (see
...
**Why fresh agent instead of resume:** Resume relies on Claude Code's internal
serialization which breaks with parallel tool calls. Fresh agents with explicit
state are more reliable and maintain full context.

**Checkpoint in parallel context:** If a plan in a parallel wave has a
checkpoint:
...
- Wait for all agents to finish before next wave </step>
...
**Waves executed:** {N} **Plans completed:** {M} of {total}
...
If user approves → continue to update_roadmap. If user reports issues → treat as
gaps_found.
...
**Score:** {N}/{M} must-haves verified **Report:**
...
2. Creates additional plans (04, 05, etc.) with `gap_closure: true` to close
   gaps
...
User stays in control at each decision point. </step>
...
**No context bleed.** Orchestrator never reads workflow internals. Just paths
and results. </context_efficiency>
...
- Last completed plan
- Current wave
- Any pending checkpoints </resumption>
