...
Use Bash (not Glob) to check—Glob respects .gitignore but .planning/ is often
gitignored:
...
If missing both ROADMAP.md and PROJECT.md: suggest `/gsd:new-project`. </step>
...
- Read `.planning/PROJECT.md` for current state (What This Is, Core Value,
  Requirements)
...
- Check for active debug sessions:
  `ls .planning/debug/*.md 2>/dev/null | grep -v resolved | wc -l` </step>
...
- Handoff file exists → mention it, offer `/gsd:resume-work` </step>
