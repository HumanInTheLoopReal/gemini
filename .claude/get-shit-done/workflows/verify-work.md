...
User tests, Claude records. One test at a time. Plain text responses. </purpose>
...
Continue to `create_uat_file`. </step>
...
Wait for user response (plain text, no AskUserQuestion). </step>
...
tests → Go to `complete_session` </step>
...
simultaneously, so overhead is minimal and fixes are more accurate. </step>
...
- **PLANNING INCONCLUSIVE:** Report and offer manual intervention </step>
...
- **ISSUES FOUND:** Proceed to `revision_loop` </step>
