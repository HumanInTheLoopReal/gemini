...
skipped or exists), spawn gsd-planner agent, verify plans with gsd-plan-checker,
iterate until plans pass or max iterations reached, present results.
...
**If not found:** Error with available phases. **If found:** Extract phase
...
**If `workflow.research` is `false` AND `--research` flag NOT set:** Skip to
step 6.
...
Answer: "What do I need to know to PLAN this phase well?" </objective>
...
**Phase context (if any):** {phase_context} </context>
...
**If exists:** Offer: 1) Continue planning (add more plans), 2) View
existing, 3) Replan from scratch. Wait for response.

## 7. Read Context Files

Read and store context file contents for the planner agent. The `@` syntax does
not work across Task() boundaries - content must be inlined.
...
**Requirements (if exists):** {requirements_content}

**Phase Context (if exists):** {context_content}

**Research (if exists):** {research_content}

**Gap Closure (if --gaps mode):** {verification_content} {uat_content}
...
**Phase:** {phase_number} **Phase Goal:** {goal from ROADMAP}
