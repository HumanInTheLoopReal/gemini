...
Shows version comparison, changelog entries for missed versions, and update
...
- Prompt: "Extract all version entries with their dates and changes. Return in
...
Note to user: "Couldn't check for updates (offline or GitHub unavailable).
Showing local changelog." </step>
...
3. **Extract entries between** - All version sections from latest down to (but
   not including) installed
...
**Breaking changes:** Surface prominently with **BREAKING:** prefix in the
output. </step>
